...
const publicSupabaseAnonKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXAiYmFzZSIsInJlZiI6ImxqbXp1anJ6ZGh3bXZ2YXBhamxyIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODEzOTM1NTUsImV4cCI6MjA5Njk2OTU1NX0.WdtYmTX0ll-mYJyusd7nxCb33GO4n6cLqcqb3ktRMLI';
const supabaseUrl = process.env.NEXT_PUBLIC_OGN_APP_SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL || publicSupabaseUrl;
const supabaseAnonKey = process.env.NEXT_PUBLIC_OGN_APP_SUPABASE_ANON_KEY || process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || publicSupabaseAnonKey;
...
export function AppConsole() {
  ...
  async function publishMedia() {
    ...
    if (!media.fileUrl && !media.externalUrl.trim()) return setNotice('Upload a media file or paste an external URL first.');
  ...
  return (
    <section className="bg-white">
      <style jsx global>{`
        .field { width: 100%; border-radius: 0.75rem; border: 1px solid #e5e7eb; padding: 0.75rem 1rem; color: #111827; outline: none; }
        .field:focus { border-color: #f59e0b; box-shadow: 0 0 0 3px rgba(245, 158, 11, 0.16); }
        .primary, .secondary, .mini, .miniDanger { border-radius: 999px; font-weight: 700; transition: all 160ms ease; }
        .primary { width: 100%; background: #f59e0b; color: white; padding: 0.75rem 1.25rem; }
        .primary:hover { background: #d97706; }
        .secondary { width: 100%; border: 1px solid #fcd34d; color: #92400e; padding: 0.75rem 1.25rem; }
        .secondary:hover { background: #fffbeb; }
        .mini, .miniDanger { border: 1px solid #e5e7eb; padding: 0.375rem 0.75rem; font-size: 0.75rem; }
      ...
      <div className="mx-auto max-w-6xl px-4 py-12">
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_360px]">
          ...
          <aside className="space-y-8">
            ...
            <Panel title="Store Review Readiness" icon={<FileVideo className="h-5 w-5" />}><ul className="space-y-3 text-sm text-gray-700">{['Privacy, Support, and Terms pages are live', 'Admins can upload covers, audio, video, books, articles, PDFs, and media files', 'Events publish to Announcements and push', 'Admins can pause, mute, remove, and role users', loading ? 'Loading app records...' : 'Back office connected to app backend'].map((label) => <li key={label} className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-green-700" />{label}</li>)}</ul></Panel>
          </aside>
        </div>
      </div>
    </section>
